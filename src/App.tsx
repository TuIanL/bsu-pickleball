  ...
  return (
    <PageFrame>
      <section className="grid min-h-[calc(100vh-8rem)] gap-8 lg:grid-cols-[0.92fr_1.08fr] lg:items-center">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-[#54FE49]/25 bg-[#54FE49]/10 px-4 py-2 text-sm font-bold text-[#C9FFC5]">
            <Sparkles size={16} aria-hidden="true" />
            AI 比赛分析 · 视频优先产品原型
          </div>
          <h1 className="mt-7 max-w-4xl text-5xl font-black leading-[0.98] text-white sm:text-6xl xl:text-7xl">
          ...
          <div className="mt-10 grid max-w-2xl grid-cols-3 gap-3">
            {[
              ["24", "indexed rallies"],
              ["82", "performance score"],
              ["4", "report layers"],
            ].map(([value, label]) => (
              <div className="rounded-2xl border border-white/10 bg-white/[0.055] p-4" key={label}>
                <strong className="block text-3xl font-black text-[#54FE49]">{value}</strong>
        ...
        <div className="relative">
          <div className="absolute -inset-4 rounded-[2.5rem] bg-[#54FE49]/10 blur-3xl" />
          <div className="relative">
            <VideoAnalysisCard
              compact
      ...
      <section className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {overviewCards.map((card) => (
          <button
            className="sport-card group p-5 text-left transition hover:-translate-y-1 hover:border-[#54FE49]/35"
            key={card.id}
            onClick={() => onNavigate(card.path)}
            type="button"
          >
            <span className="text-xs font-black uppercase tracking-[0.16em] text-[#54FE49]">{card.metric}</span>
            <strong className="mt-4 block text-xl font-black text-white">{card.title}</strong>
            <p className="mt-3 text-sm leading-6 text-slate-400">{card.body}</p>
            <span className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-slate-200 transition group-hover:text-[#54FE49]">
              Open
  ...
  return (
    <PageFrame>
      <section className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="inline-flex items-center gap-2 text-sm font-bold uppercase tracking-[0.18em] text-[#54FE49]">
            <Camera size={16} aria-hidden="true" />
            AI Video Analytics
          </p>
          <h1 className="mt-3 text-4xl font-black text-white sm:text-5xl">比赛分析 Dashboard</h1>
          <p className="mt-4 max-w-3xl text-base leading-7 text-slate-400">
            视频回放是入口，报告和训练建议是下一步。这里用 mock 视频层展示球路轨迹、站位热力和关键片段。
          </p>
        </div>
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {reportActions.map((action) => (
          <button
            ...
            type="button"
          >
            <span className="grid size-10 place-items-center rounded-2xl border border-white/10 bg-[#54FE49]/10 text-[#54FE49]">
              <LineChart size={18} aria-hidden="true" />
            </span>
            <strong className="mt-4 block text-lg font-black text-white">{action.title}</strong>
            <p className="mt-2 text-sm leading-6 text-slate-400">{action.description}</p>
            <span className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-slate-300 group-hover:text-[#54FE49]">
              Open report
  ...
  return (
    <section className="sport-card p-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#54FE49]">AI Coach Notes</p>
          <h2 className="mt-2 text-xl font-black text-white">可执行洞察</h2>
        </div>
        <Brain className="text-[#54FE49]" size={22} aria-hidden="true" />
      </div>
...
function HighlightsCard({ onNavigate }: { onNavigate: (path: AppPath | "/reports/landing") => void }) {
  return (
    <section className="sport-card p-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Highlights</p>
          <h2 className="mt-2 text-xl font-black text-white">关键片段</h2>
        </div>
      ...
      <div className="mt-5 grid gap-3">
        {highlights.map((highlight) => {
          ...
          return (
            <button
              className="rounded-2xl border border-white/10 bg-white/[0.045] p-4 text-left transition hover:border-white/20 hover:bg-white/[0.075]"
              key={highlight.id}
    ...
    <PageFrame>
      <section className="sport-card overflow-hidden">
        <div className="grid gap-6 p-6 lg:grid-cols-[1fr_0.45fr] lg:p-8">
          <div>
            <button
              className="mb-6 inline-flex items-center gap-2 text-sm font-bold text-slate-400 transition hover:text-[#54FE49]"
            ...
            <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#54FE49]">{definition.eyebrow}</p>
            <h1 className="mt-3 max-w-4xl text-4xl font-black text-white sm:text-5xl">{definition.title}</h1>
            <p className="mt-4 max-w-3xl text-base leading-7 text-slate-400">{definition.summary}</p>
          </div>
          <div className="rounded-3xl border border-[#54FE49]/25 bg-[#54FE49]/10 p-6">
            <span className="text-sm font-bold text-[#C9FFC5]">{definition.heroMetricLabel}</span>
            <strong className="mt-4 block text-5xl font-black text-[#54FE49]">{definition.heroMetric}</strong>
            <button className="mt-6 green-button w-full" onClick={() => onNavigate("/training")} type="button">
              Go to related training
      ...
      <section className="mt-6 grid gap-4 lg:grid-cols-[0.78fr_1.22fr]">
        <article className="sport-card p-5 sm:p-6">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#54FE49]">Training Bridge</p>
          <h2 className="mt-2 text-2xl font-black text-white">{definition.trainingLink}</h2>
  ...
  return (
    <section className="sport-card p-5 sm:p-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#54FE49]">Recommended Drills</p>
          <h2 className="mt-2 text-2xl font-black text-white">数据变成训练任务</h2>
        </div>
      ...
      {drills.map((drill) => (
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <span className="rounded-full border border-[#54FE49]/25 bg-[#54FE49]/10 px-2.5 py-1 text-xs font-black text-[#C9FFC5]">
              {drill.difficulty}
            </span>
            <span className="text-xs font-bold text-slate-500">{drill.duration}</span>
          </div>
          <h3 className="mt-4 text-lg font-black text-white">{drill.title}</h3>
          <p className="mt-3 text-sm leading-6 text-slate-400">{drill.goal}</p>
          <p className="mt-4 rounded-2xl bg-black/22 p-3 text-xs leading-5 text-slate-400">{drill.evidence}</p>
    ...
    <PageFrame>
      <section className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
        <div>
          ...
          <h1 className="mt-3 text-4xl font-black text-white sm:text-5xl">训练建议页</h1>
        ...
        <div className="sport-card p-5">
          <div className="flex items-center gap-3">
            <CheckCircle2 className="text-[#54FE49]" size={24} aria-hidden="true" />
            <div>
              <strong className="text-xl font-black text-white">Learn · Practice · Evaluate</strong>
              <p className="mt-1 text-sm text-slate-400">报告问题 → 教学内容 → 训练任务 → 下次复测目标</p>
            </div>
          </div>
          <div className="mt-5 grid gap-3">
            {trainingRecommendations.map((item) => (
              <article className="rounded-2xl border border-white/10 bg-white/[0.045] p-4" key={item.id}>
                <strong className="text-white">{item.title}</strong>
                <p className="mt-2 text-sm leading-6 text-slate-400">{item.practiceTask}</p>
                ...
                <p className="mt-2 text-xs font-semibold text-[#C9FFC5]">{item.nextTarget}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="mt-6 grid gap-6 lg:grid-cols-[0.75fr_1.25fr]">
        <article className="sport-card p-5 sm:p-6">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#54FE49]">Teaching Content</p>
          <h2 className="mt-2 text-2xl font-black text-white">教学视频与动作对标占位</h2>
          <div className="mt-5 aspect-video rounded-3xl border border-white/10 bg-black/25 p-5">
            <div className="flex h-full items-center justify-center rounded-2xl border border-dashed border-white/15 bg-white/[0.035]">
              <div className="text-center">
                <span className="mx-auto grid size-14 place-items-center rounded-full bg-[#54FE49]/15 text-[#54FE49]">
                  <Play size={26} fill="currentColor" aria-hidden="true" />
                </span>
                <strong className="mt-4 block text-white">Backhand dink comparison</strong>
    ...
    <PageFrame>
      <section className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        <div>
          <p className="inline-flex items-center gap-2 text-sm font-bold uppercase tracking-[0.18em] text-[#54FE49]">
            ...
            Phase 2 Hardware Fusion
          </p>
          <h1 className="mt-3 text-4xl font-black text-white sm:text-5xl">{hardwarePreview.phaseLabel}</h1>
          <p className="mt-4 max-w-2xl text-base leading-7 text-slate-400">{hardwarePreview.disclaimer}</p>
        ...
        <div className="sport-card p-5 sm:p-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">TENG Array</p>
              <h2 className="mt-2 text-2xl font-black text-white">3x3 甜区触点分布</h2>
            </div>
            <span className="inline-flex items-center gap-2 rounded-full border border-[#54FE49]/25 bg-[#54FE49]/10 px-3 py-2 text-xs font-black text-[#C9FFC5]">
              <ShieldCheck size={14} aria-hidden="true" />
              Simulated
            </span>
          </div>
          <div className="mt-6 grid grid-cols-3 gap-3 rounded-[2rem] border border-white/10 bg-black/25 p-4">
            {hardwarePreview.sweetZone.map((cell) => (
              <span
                className={`aspect-square rounded-2xl border ${
                  cell.id === hardwarePreview.highlightedCellId
                    ? "border-[#54FE49] bg-[#54FE49]"
                    : "border-white/10 bg-[#54FE49]/10"
                }`}
                key={cell.id}
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {hardwarePreview.metrics.map((metric) => (
          <article className="sport-card p-5 transition hover:-translate-y-1 hover:border-[#54FE49]/35" key={metric.id}>
            <Gauge className="text-[#54FE49]" size={20} aria-hidden="true" />
      ...
      <section className="mt-6 grid gap-4 lg:grid-cols-2">
        {hardwarePreview.fusionPoints.map((point) => (
          <article className="sport-card p-5 sm:p-6" key={point.insight}>
            ...
            <p className="mt-5 rounded-2xl border border-[#54FE49]/20 bg-[#54FE49]/10 p-4 text-sm font-semibold leading-6 text-[#C9FFC5]">
              {point.insight}
            </p>
          </article>
        ))}
      ...
      <section className="mt-6 sport-card p-5 sm:p-6">
        <div className="flex items-start gap-4">
          <BadgeCheck className="mt-1 text-[#54FE49]" size={23} aria-hidden="true" />
          <div>
            <h2 className="text-2xl font-black text-white">未来数据替换路径</h2>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-400">
              当前页面数值全部来自本地模拟数据。未来真实 TENG 与 IMU 数据可以替换硬件 mock object，
...
function FusionBlock({ icon, label, value }: { icon: ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.045] p-4">
      <span className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.14em] text-[#54FE49]">
        {icon}
        {label}
      </span>
      <strong className="mt-3 block text-base leading-6 text-white">{value}</strong>
    </div>
  );
}
