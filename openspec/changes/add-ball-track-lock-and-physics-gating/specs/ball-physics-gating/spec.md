...
- **THEN** the prediction SHALL fall back to the last-2-point linear extrapolation

#### Scenario: Prediction with no history

- **WHEN** the tracker has fewer than 2 accepted points
- **THEN** no prediction SHALL be made
- **AND** `predicted_position` SHALL be None
...
- **THEN** `dynamic_gate_pixels` SHALL be computed as `base_gate_pixels + speed_factor * recent_speed_px_per_frame + missing_factor * missing_frames`
...
- **AND** the gate SHALL NOT be wider than `max_gate_pixels` (hard upper cap)
...
- **THEN** a simple region multiplier SHALL be used: near-court (lower image region) × 1.2-1.5, far-court (upper image region) × 0.8-1.0
...
The tracker SHALL use different scoring weights for each track state. Detector confidence, prediction proximity, motion consistency, candidate size consistency, and court region SHALL be weighted according to the current state.
...
- **THEN** `missing_frame_gate_multiplier` SHALL be applied to widen the acceptable search area
- **AND** the tracker SHALL prioritise returning to LOCKED state over strict physical consistency

### Requirement: Static false-positive suppression with player motion context

The tracker SHALL detect and suppress candidates that remain nearly stationary while player motion is active. This extends the existing stationary blacklist with a new player-motion-aware pathway.

#### Scenario: Stationary candidate with active player motion is suppressed

- **WHEN** a candidate remains within `static_motion_pixels` for `static_window_frames`
- **AND** player motion exceeds `player_motion_min_pixels` during the same window
- **AND** the current timeline segment is not marked as non-play
- **THEN** the tracker SHALL mark the candidate region as a static false positive
- **AND** future candidates in the same region SHALL receive a `static_false_positive_penalty`
...
- **THEN** the tracker SHALL fall back to the existing stationary blacklist behavior
- **AND** the player-motion-aware static suppression SHALL be skipped without error

#### Scenario: Static false-positive penalty in candidate scoring

- **WHEN** a candidate falls into a player-motion-aware static false-positive region
- **THEN** the tracker SHALL subtract `static_false_positive_penalty` from the candidate's final score
- **AND** the penalty value SHALL be configurable in `BallTrackerConfig`
...
- **THEN** the frame SHALL emit debug metadata including: `track_state`, `accepted_candidate_id`, `predicted_position`, and a list of all candidates with their `raw_confidence`, `final_score`, `distance_to_prediction`, `passed_physics_gate`, and `rejection_reason`

#### Scenario: Missing frame emits debug data

- **WHEN** the tracker produces a missing frame (no candidate accepted)
- **THEN** the frame SHALL emit debug metadata including: `track_state`, `predicted_position`, `overall_decision`, `missing_frames`, and `rejection_reason`
...
- **THEN** the rejection reason SHALL be one of: `physics_gate_rejected`, `static_false_positive`, `too_far_from_prediction`, `jump_too_large`, `speed_too_high`, `low_final_score`, `accepted`
