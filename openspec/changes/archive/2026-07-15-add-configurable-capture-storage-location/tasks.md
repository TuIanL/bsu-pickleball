...
- [x] 9.1 验证默认位置单摄：视频、事件、标记、时间线、manifest 和分析产物均可从同一会话目录读取
- [x] 9.2 验证自定义移动硬盘位置单摄和双摄：重启应用后重新选择原目录不会产生重复 captures 或覆盖旧录制
- [x] 9.3 验证同一天多次录制均使用独立 capture_take_id 目录
- [x] 9.4 验证拔出移动硬盘后录制立即失败、部分内容保留、SQLite 索引可查询且不触发分析
- [x] 9.5 验证历史默认目录录制仍可查询、播放、分析和删除
