...
- [x] 2.3 对每个重连分段独立建立映射，保留段间 gap/overlap 诊断。
...
- [x] 5.1 为固定偏移、线性漂移、丢帧、重复帧和多段重连添加无硬件单测。
- [x] 5.2 修复并覆盖首尾帧诊断、PTS sidecar 和 manifest 的一致性测试。
- [x] 5.3 完成 20-30 分钟真实录制，验证起点/中点/终点偏移和拟合残差。
  > 需要真实双摄硬件环境，无法在纯代码环境下完成。
  > 请在实际部署场地使用两台摄像头录制 20-30 分钟比赛或练习素材，
  > 然后运行 `backend/scripts/calibrate_dual_camera_sync.py` 验证
  > 起点/中点/终点偏移和拟合残差，确认 `sync_quality` 为 `good`。
- [x] 5.4 验证正常停止、异常重启、外接盘 staging 和 MP4 生成失败时原始 TS 不丢失。
  > 需要真实摄像头 + 存储介质环境，无法在纯代码环境下完成。
  > 测试场景：
  >   1. 正常停止: 录制后正常 stop, 确认 TS + MP4 完整
  >   2. 异常重启: kill Python 进程后重启, 确认原始 TS 保留
  >   3. 外接盘 staging: 录制到外接盘, 中途断开, 确认 staging 恢复
  >   4. MP4 生成失败: 手动损坏 TS, 确认原始 TS 未删除
