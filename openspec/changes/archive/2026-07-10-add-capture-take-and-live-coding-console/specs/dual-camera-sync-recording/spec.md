...
系统 MUST 在双摄录制启动时自动创建 CaptureTake 记录。

#### Scenario: 双摄录制创建 CaptureTake

- **WHEN** 双摄同步录制启动成功
- **THEN** 系统 SHALL 创建 CaptureTake 记录
- **AND** `capture_mode` SHALL 设置为 `dual`
- **AND** `source_session_type` SHALL 设置为 `sync_recording`
- **AND** `source_session_id` SHALL 设置为 SyncRecordingSession ID
- **AND** `status` SHALL 设置为 `recording`
- **AND** 系统 SHALL 为每个摄像头创建 CaptureTrack
- **AND** 第一个摄像头的 `role` SHALL 为 `primary`，`offset_source` SHALL 为 `measured`
- **AND** 第二个摄像头的 `role` SHALL 为 `secondary`
- **AND** 第二个摄像头的 `offset_ms` SHALL 优先根据两路实际文件 `started_at` 差值计算
- **AND** 无法自动测量时 `offset_ms` SHALL 为 0，`offset_source` SHALL 为 `assumed`，`sync_quality` SHALL 为 `unknown`

#### Scenario: 双摄录制停止关闭 CaptureTake

- **WHEN** 双摄同步录制停止或失败或取消
- **THEN** 系统 SHALL 更新对应 CaptureTake 的 `status`
- **AND** 系统 SHALL 记录 `ended_at` 和 `duration_ms`
- **AND** 如果有进行中的区间，系统 SHALL 关闭这些区间（状态为 `inferred`）

#### Scenario: 双摄 CaptureTake ID 返回给前端

- **WHEN** 双摄录制启动成功并创建 CaptureTake
- **THEN** 响应 SHALL 包含 `capture_take_id`
- **AND** 前端 SHALL 使用该 ID 进行后续的事件打点

### Requirement: 双摄事件打点支持

系统 MUST 支持在双摄录制时创建时间线事件。

#### Scenario: 双摄录制中显示事件按钮

- **WHEN** 用户处于双摄录制状态
- **THEN** 前端 SHALL 显示事件打点按钮
- **AND** 按钮 SHALL 使用 CaptureTake ID 而非 RecordingSession ID

#### Scenario: 双摄事件关联 CaptureTake

- **WHEN** 用户在双摄录制中创建事件
- **THEN** 事件 SHALL 关联 CaptureTake
- **AND** 事件的 `timestamp_ms` SHALL 相对 CaptureTake 开始时间
- **AND** 事件的时间戳可用于两个摄像头的视频定位
