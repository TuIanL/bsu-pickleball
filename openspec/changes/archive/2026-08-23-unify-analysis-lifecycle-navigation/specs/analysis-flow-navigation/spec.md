...
## Purpose
定义 transient analysis flow（Setup → Calibration → Progress）的统一导航上下文契约：`return` 即 origin、origin 由 URL 推导、origin 完整转发、统一创建后进入进度页、origin 化返回与完成去向、Library-origin Sidebar 高亮、`source` 词汇表收敛，以及 Live Analysis Projection（运行时快照与素材身份解耦、共享 watch、真实进度）。
