...
系统 MUST 允许用户删除没有录制关联且不在进行中的 Field Session。

#### Scenario: 删除空采集任务
- **WHEN** 用户请求删除一个不存在录制关联且 `status` 不是 `live` 的 Field Session
...
系统 MUST 在前端提供 Field Session 采集控制台，用于在任务上下文中操作摄像头预览和录制。
