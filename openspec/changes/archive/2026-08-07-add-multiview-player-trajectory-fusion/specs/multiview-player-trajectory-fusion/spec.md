...
定义观测质量评估（ViewIntrinsicQuality + PairConsistency）、位置融合状态机、`GlobalTrackFilter` 的 predict/update 时序、`FusedPlayerTrajectoryArtifact` 及 metric eligibility，保证多视角融合输出单一全局球员轨迹，并在不可信时显式回退。
