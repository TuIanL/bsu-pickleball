...
定义 Multi-view 分析的输入组合与权威时间契约：`MultiViewFusionRun` 持有的输入、`dual_camera_sync_calibration.v1` 同步 authority、Canonical Timeline 与 pairing tolerance，确保 Fusion 层的时间语义可追踪、可门控，且不会在同步质量不足时伪装为 synchronized fusion。
