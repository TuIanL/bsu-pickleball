...
- **WHEN** a detector returns player candidates for a processed video frame
...
- **WHEN** a detector returns classes outside the configured player target map
...
- **WHEN** a detector returns a target below the configured confidence threshold for the player class
...
- **WHEN** a detector can emit supported player classes for a processed frame
...
The backend SHALL distinguish unavailable, skipped, no-detection, partial, and available states for player-focused perception outputs.

#### Scenario: Model inference is disabled
- **WHEN** optional multi-target inference is disabled by configuration
...
- **WHEN** the detector produces usable player detections but no other supported target detections
- **THEN** the pipeline reports class-specific availability so player overlays can remain usable while unsupported targets stay unavailable
