...
定义多视角分析任务的编排契约：Parent/Source Job 关系、独立编排状态维度、`is_runnable()` 统一 claim 判定、`MultiViewAnalysisCoordinator`、dedicated/owned child、应用启动对账、取消/删除级联与 Child 外部保护、MultiView preflight。它解决"一次双摄录制如何变成一个用户可见的 Parent 分析任务、两路 Source Job 如何被自动创建与隐藏、Parent 何时可被执行、失败如何降级"的编排问题。
