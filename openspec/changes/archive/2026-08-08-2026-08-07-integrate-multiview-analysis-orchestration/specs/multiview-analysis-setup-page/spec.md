...
定义双摄协同分析的前端设置页 `MultiViewAnalysisSetupPage`（路由 `/capture/takes/:captureTakeId/analyze`）与双摄主 CTA。页面以 CaptureTake 为核心，四阶段完成素材检查、A/B 两路标定与最终确认，一次创建唯一一个 multiview Parent 任务。同时定义 CourtOrientation 的产品化确认（端 A/B 而非算法枚举）与现有 `cameraAngle` 错误映射的清理。
