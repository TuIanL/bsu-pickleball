...
- [ ] 4.5 增加双摄目录结构、历史目录复用、同日多次录制和双路失败清理测试
...
- [ ] 5.4 增加页面刷新、Outbox 迟到事件、停止超时和事件文件写入失败的测试
...
- [ ] 6.4 让 AnalysisPipelineResult、artifact API 和前端展示通过 SQLite 索引解析逻辑 artifact 引用，不暴露绝对路径
- [ ] 6.5 增加视频、JSON、JSONL、图像和分析叠加视频均位于会话目录的测试
...
- [ ] 7.5 增加拔出移动硬盘、目录权限变化、空间耗尽和故障后恢复查询的单元/集成测试
