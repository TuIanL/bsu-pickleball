...
系统 SHALL 为每次正式切分持久化 `MatchStateSegmentationRun`。Run MUST 关联 CaptureTake 和 planning job，并记录状态、模型/输入/sync provenance、artifact hash、unknown rate、片段数、`supersedes_run_id` 和开始/结束时间。成功或正常零回合运行 MUST 写出带 run id 与 hash 的不可变 `AnalysisWindowPlan`；分析 Parent MUST 绑定 run id 和 plan hash，后续消费者 SHALL 仅使用该计划。
...
#### Scenario: 运行中出现新版本
- **WHEN** Job A 已绑定 Run 1，随后同一 CaptureTake 成功发布 Run 2
- **THEN** Job A 的指标、事件归属和报告 SHALL 继续使用 Run 1 的窗口计划
- **AND** MUST NOT 改为查询当前最新的自动 Segment
