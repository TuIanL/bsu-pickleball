# ball-track-state-machine Specification

## Purpose
Define a ball track state machine (SEARCHING / TENTATIVE / LOCKED / LOST) with state-dependent candidate selection behavior, enabling the tracker to lock onto stable trajectories and reject physically implausible candidates during locked state.
## Requirements
