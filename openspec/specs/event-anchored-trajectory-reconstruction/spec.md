...
## Purpose
定义事件锚定的 2.5D 视觉重建：在图像空间鲁棒拟合，以可信事件锚点在球场坐标重建展示轨迹，并按段类型生成事件边界感知的高度模型。不声称真实三维测量。
## Requirements
...
系统 SHALL 根据可用空间锚点数量选择重建模式，锚点不足时不得伪装高可信球场空间重建。
...
#### Scenario: 单锚点模式
- **WHEN** 仅一端有空间锚点（bounce→loss、hit→loss、unknown→bounce）
- **THEN** 系统 SHALL 执行 `single_anchor_warp` 低可信重建
- **AND** 锚点端严格对齐，另一端使用 pseudo path 相对位移
- **AND** 仅显示有观测支持的区间，越靠近未知端透明度越低，不生成精确终点
- **AND** 该段总体质量上限 SHALL 受限

#### Scenario: 无锚点模式
- **WHEN** 段两端均无空间锚点（loss→loss、unknown→unknown）
- **THEN** 系统 SHALL 标记 `reconstruction_mode = image_only` 且 `status = insufficient_spatial_anchors`
- **AND** 该段 SHALL NOT 出现在默认球场空间视图
- **AND** 该段 SHALL 仍可在原始检测或图像拟合调试模式中显示
