...
系统 SHALL 在停止路由完成媒体停止后正确组装并返回完整的 CaptureStopResult，而不得因响应组装错误返回 HTTP 500。
...
- **WHEN** `session_service.stop_session()` 成功完成媒体停止
- **THEN** 停止端点 SHALL 返回 HTTP 200 和完整的 `CaptureStopResult`
- **AND** 响应 MUST 包含 `capture_take`、单条 `tracks`、`default_analysis_video_id` 和 `analysis_available`
