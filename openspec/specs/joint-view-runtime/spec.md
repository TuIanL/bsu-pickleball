# joint-view-runtime Specification

## Purpose
JointViewRuntime prepare/complete 窄接口：解帧恰好一次 + 转发 tracking session 事务两阶段，主循环不越过 runtime 直接解帧、same-tick 阶段不重复解同一 source frame。
