...
定义双摄分析的编排契约：编排字段与状态独立维度、按模式与 prerequisite 的 `is_runnable()` 判定、Coordinator 事件驱动编排与启动 reconciliation、取消与删除级联、两套 orchestrationStatus 冻结，以及窗口传播、sync authority preflight 与阶段状态的单一事实源。
## Requirements
