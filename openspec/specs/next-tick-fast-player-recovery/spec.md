# next-tick-fast-player-recovery Specification

## Purpose
available-miss fast path：`ViewBinding` 独立于 `visibility` 的可用性维度记账（`consecutive_available_misses` 等），使 guidance 在"上一 canonical tick 该 view 被成功尝试但该 global 无 AssociationUpdate"时于下一 tick 提前触发，无需等待 `binding_weak_after_ms=300ms`。共享 predicate `is_target_recovery_eligible` 同时驱动 recovery opportunity/episode 建立与 guidance 触发（消灭幽灵 guidance）；`GuidanceDecision` 拆 trigger/reason；非 same-tick 语义，cooldown 与安全门原样保持。
