...
def test_player_motion_static_false_positive_suppression():
    ...
    detector = StubBallDetector(
        [
            [candidate(200, 300, 0.9)]
            for _ in range(35)  # 35 frames at same spot
        ]
    )
    tracker = BallTracker(
        detector,
    ...
    for i in range(30):
        tracker.update(frame(), i, i * 0.033, player_motion_pixels=50.0)
...
def test_no_player_motion_fallback_to_existing_behavior():
    ...
    detector = StubBallDetector([[candidate(200, 300, 0.9)] for _ in range(35)])
    tracker = BallTracker(
        detector,
        make_config(stationary_window_frames=30, stationary_radius_pixels=5.0),
    )
