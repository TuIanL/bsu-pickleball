...
print("=== Segment 编辑 ===")
db = fresh_db()
from app.models.capture_segment import EditStatus  # noqa: E402
from app.services import capture_segment_service as seg_svc  # noqa: E402
from app.services import segment_edit_service as edit_svc  # noqa: E402
...
game_seg = seg_svc.create_segment(
    db, capture_take_id=tid, segment_type="game", ordinal=1, start_ms=1000, parent_segment_id=set_seg.id, label="第1局"
)
r1 = seg_svc.create_segment(
    db,
    capture_take_id=tid,
    segment_type="rally",
    ordinal=1,
    start_ms=2000,
    parent_segment_id=game_seg.id,
    label="第1分",
)
seg_svc.close_segment(db, r1, end_ms=5000, status="closed")
r2 = seg_svc.create_segment(
    db,
    capture_take_id=tid,
    segment_type="rally",
    ordinal=2,
...
# merge
r3_data = seg_svc.create_segment(
    db,
    capture_take_id=tid,
    segment_type="rally",
    ordinal=3,
...
r4_data = seg_svc.create_segment(
    db,
    capture_take_id=tid,
    segment_type="rally",
    ordinal=4,
...
print("\n=== AnalysisBatch ===")
db = fresh_db()
from app.services import analysis_batch_service as batch_svc  # noqa: E402
...
game_seg = seg_svc.create_segment(
    db, capture_take_id=tid, segment_type="game", ordinal=1, start_ms=1000, parent_segment_id=set_seg.id, label="G1"
)
r1 = seg_svc.create_segment(
    db, capture_take_id=tid, segment_type="rally", ordinal=1, start_ms=2000, parent_segment_id=game_seg.id, label="R1"
)
seg_svc.close_segment(db, r1, end_ms=5000, status="closed")
r2 = seg_svc.create_segment(
    db, capture_take_id=tid, segment_type="rally", ordinal=2, start_ms=5000, parent_segment_id=game_seg.id, label="R2"
)
seg_svc.close_segment(db, r2, end_ms=8000, status="closed")
db.commit()
...
# 拒绝混选不同类型
game_r = seg_svc.create_segment(
    db, capture_take_id=tid, segment_type="game", ordinal=2, start_ms=8000, parent_segment_id=set_seg.id, label="G2"
)
...
from app.services.analysis_pipeline import AnalysisPipeline, _TrackingRunOutput  # noqa: E402
