...
class SessionTimelineEvent(Base):
    ...
    occurred_at: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, default=lambda: datetime.now(timezone.utc)                       # 事件发生的UTC时间
    )

    event_type: Mapped[TimelineEventType] = mapped_column(
        Enum(TimelineEventType), nullable=False                                                    # 事件类型
    )
    source: Mapped[TimelineEventSource] = mapped_column(
        Enum(TimelineEventSource), nullable=False, default=TimelineEventSource.manual              # 事件来源
    )
