...
# 录制单元状态枚举
class CaptureTakeStatus(enum.StrEnum):
...
# 源会话类型枚举
class SourceSessionType(enum.StrEnum):
...
class CaptureTake(Base):
    ...
    status: Mapped[CaptureTakeStatus] = mapped_column(
        Enum(CaptureTakeStatus),
        nullable=False,
        default=CaptureTakeStatus.recording,  # 录制状态
    )
    started_at: Mapped[datetime] = mapped_column(
        DateTime,
        nullable=False,
        default=lambda: datetime.now(UTC),  # 开始时间
    )
