...
class CaptureTrack(Base):
    ...
    slot: Mapped[CaptureTrackSlot] = mapped_column(
        Enum(CaptureTrackSlot), nullable=False, default=CaptureTrackSlot.cam_1                # 插槽位置
    )
    analysis_role: Mapped[AnalysisRole] = mapped_column(
        Enum(AnalysisRole), nullable=False, default=AnalysisRole.default                      # 分析角色
    )
    ...
    offset_source: Mapped[OffsetSource] = mapped_column(
        Enum(OffsetSource), nullable=False, default=OffsetSource.assumed                      # 偏移来源
    )
    sync_quality: Mapped[SyncQuality] = mapped_column(
        Enum(SyncQuality), nullable=False, default=SyncQuality.unknown                        # 同步质量
    )
