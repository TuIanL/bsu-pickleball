...
class LiveCodingState(Base):
    ...
    capture_take_id: Mapped[str] = mapped_column(
        String(64),
        ForeignKey("capture_takes.id", ondelete="CASCADE"),
        primary_key=True,  # 所属录制单元ID（主键/外键，级联删除）
    )
    ...
    match_phase: Mapped[str] = mapped_column(
        String(32), nullable=False, default="idle"
    )  # 比赛阶段（idle/playing/break 等）
    ...
    )  # 计分模式（"side_out_singles_v1" / "manual" / "none"）
