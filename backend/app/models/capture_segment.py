...
class CaptureSegment(Base):
    ...
    __table_args__ = (
        CheckConstraint("end_ms IS NULL OR end_ms >= start_ms", name="ck_segment_end_after_start"),  # 结束时间不能早于开始时间
    ...
    capture_take_id: Mapped[str] = mapped_column(
        String(64), ForeignKey("capture_takes.id", ondelete="RESTRICT"), nullable=False              # 所属录制单元ID（外键，禁止级联删除）
    )
    ...
    parent_segment_id: Mapped[str | None] = mapped_column(
        String(64), ForeignKey("capture_segments.id", ondelete="SET NULL"), nullable=True            # 父区间ID（用于层级结构，如盘→局→回合）
    )
    ...
    edit_status: Mapped[EditStatus] = mapped_column(
        Enum(EditStatus), nullable=False, default=EditStatus.active                                 # 编辑状态
    )
    ...
    status: Mapped[SegmentStatus] = mapped_column(
        Enum(SegmentStatus), nullable=False, default=SegmentStatus.open                             # 区间状态
    )
