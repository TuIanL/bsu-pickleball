...
class CalibrationResult(BaseModel):
    ...
    inverse_homography: Optional[HomographyMatrix] = None  # 逆向矩阵：球场→图像
...
class ReferenceLineDiagnostics(BaseModel):
    ...
    rejection_reason: Optional[str] = None           # 若被拒，记录原因
    summary: str = ""
...
class AutomaticCalibrationResponse(BaseModel):
    ...
    detail: str
    suggestion_id: Optional[str] = None              # 建议 id（接受时用来保存）
    ...
    preview_image_url: Optional[str] = None          # 预览图地址
    calibration_id: Optional[str] = None             # 接受后生成的正式标定 id
