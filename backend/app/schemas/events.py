...
class ServeSignalScores(BaseModel):
    ...
    baseline_position_score: Optional[float] = Field(default=None, ge=0, le=1)  # 底线站位分
    pre_stillness_score: Optional[float] = Field(default=None, ge=0, le=1)     # 发球前静止分
    arm_motion_peak_score: Optional[float] = Field(default=None, ge=0, le=1)   # 手臂动作峰值分
    roi_motion_peak_score: Optional[float] = Field(default=None, ge=0, le=1)   # ROI 运动峰值分
    rally_after_score: Optional[float] = Field(default=None, ge=0, le=1)       # 发球后回合分
    receiver_waiting_score: Optional[float] = Field(default=None, ge=0, le=1)  # 接球方等待分
...
class ServeCoverageDiagnostics(BaseModel):
    ...
    coverage_ratio: Optional[float] = Field(default=None, ge=0, le=1)  # 整体覆盖率
...
class ServeEventCandidate(BaseModel):
    ...
    court_position: Optional[list[float]] = Field(default=None, min_length=2, max_length=2)  # 球场坐标 [x, y]
    court_unit: Optional[str] = None
    signals: Optional[ServeSignalScores] = None   # 各信号得分明细
...
class ServeEventsArtifact(BaseModel):
    ...
    debug_artifacts: Optional[ServeDebugArtifactRefs] = None            # 调试产物引用
    coverage: Optional[ServeCoverageDiagnostics] = None                # 覆盖度诊断
