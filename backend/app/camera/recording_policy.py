...
class RecordingPolicy(Protocol):
    ...
    def decide(self, event: TrackRuntimeEvent, snapshot: CaptureRuntimeSnapshot) -> list[CoordinatorAction]: ...
...
def _build_restart_action(track_id: str, restart_count: int) -> CoordinatorAction | None:
    ...
    return CoordinatorAction(type=CoordinatorActionType.RESTART_FAILED_TRACK, track_ids=[track_id], delay_seconds=delay)
...
class StrictSyncPolicy:
    ...
    def decide(self, event: TrackRuntimeEvent, snapshot: CaptureRuntimeSnapshot) -> list[CoordinatorAction]:
        ...
        return [
            ...
            CoordinatorAction(
                type=CoordinatorActionType.RESTART_ALL, track_ids=all_ids, delay_seconds=action.delay_seconds
            ),
        ]
