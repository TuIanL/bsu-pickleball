        ...
        *,
        ...
        try:
            f = MediaFragment(
                ...
                file_path=file_path,
                status=FragmentStatus.starting,
...
class DbProcessRegistry:
    ...
    def register_ended(self, registration_id: int, *, return_code: int, exit_reason: str, ended_at: datetime) -> None:
        db = get_session_factory()()
        try:
            db.query(FFmpegProcessRegistry).filter(
                FFmpegProcessRegistry.id == registration_id,
            ).update({"return_code": return_code, "exit_reason": exit_reason, "ended_at": ended_at})
            db.commit()
        except Exception:
            db.rollback()
        finally:
